...
export default {
  async fetch(request: Request, env: any) {
    ...
    try {
      ...
      const width = clamp64(parseInt(url.searchParams.get('width') || '768', 10));   // a bit larger default
      const height = clamp64(parseInt(url.searchParams.get('height') || '512', 10)); // nice 3:2/landscape
      ...
      const seed = url.searchParams.has('seed') ? parseInt(url.searchParams.get('seed')!, 10) : undefined;

      // ---- Build “smart” prompt
      const style = STYLES[styleKey] || STYLES['kids-3d'];
      const userPart = rawPrompt || "friendly rescue puppies in uniforms helping the city"; // smarter default than a single brand word
      ...
      const png = await env.AI.run("@cf/stabilityai/stable-diffusion-xl-base-1.0", inputs);
      ...
      // ---- helpers
      function buildPrompt(user: string, s: {prefix: string; suffix?: string}) {
        ...
        return `${s.prefix} ${user}${addDetail}${s.suffix ? ", " + s.suffix : ""}`;
      }
